...
bool ClientSideEncryption::checkPublicKeyValidity(const AccountPtr &account) const
{
    ...
    auto encryptedData = EncryptionHelper::encryptStringAsymmetric(account->e2e()->getTokenCertificate(), *account->e2e(), account->e2e()->_publicKey, data.toBase64());
    if (!encryptedData) {
...
bool ClientSideEncryption::checkEncryptionIsWorking() const
{
    ...
    auto encryptedData = EncryptionHelper::encryptStringAsymmetric(getTokenCertificate(), *this, getPublicKey(), data);
    if (!encryptedData) {
        qCWarning(lcCse()) << "encryption error";
        return false;
    }
